...
frequency_response_real = torch.Tensor(S12_real)
trained_frequency_response_real = torch.Tensor(S12_real[0:a])
testing_frequency_response_real = torch.Tensor(S12_real[a:totalSize])


frequency_response_imag = torch.Tensor(S12_imag)
trained_frequency_response_imag = torch.Tensor(S12_imag[0:a])
testing_frequency_response_imag = torch.Tensor(S12_imag[a:totalSize])
...
for i in range(n_experiments):
    ...
    actual_1, predicted_1, test_loss_1 = optimization_1.evaluate(x_test, y_test, future=20, batch_size=20)
...
plt.plot(frequency.squeeze(), S12_real)
...
outputlower_bound = torch.zeros(transformed_back_lower_bound.shape[0],)
outputupper_bound = torch.zeros(transformed_back_upper_bound.shape[0],)
...
for i in range((transformed_back_mean.shape[0])):
    ...
    outputlower_bound[i] = (transformed_back_lower_bound[i])
    outputupper_bound[i] = (transformed_back_upper_bound[i])

# %%
plt.figure()
plt.plot(frequency.squeeze(), S12_real, label='actual')
plt.plot(testing_frequency[seq_len-1:-batch_size-1], output_mean, label='predicted mean')
plt.fill_between(testing_frequency[seq_len-1:-batch_size-1], outputlower_bound, outputupper_bound, color='lightblue', label='95% Confidence region')
plt.grid(b=True)
plt.legend(loc='best')
plt.show()
...
y_real_extrapolated_lower_bound = outputlower_bound
y_real_extrapolated_upper_bound = outputupper_bound


Aa = torch.Tensor(frequency_response_real[0:a])

y_real_hat = torch.cat((Aa, output_mean), dim=0)
y_real_hat_lower_bound = torch.cat((Aa, outputlower_bound), dim=0)
y_real_hat_upper_bound = torch.cat((Aa, outputupper_bound), dim=0)
...
plt.subplot(2, 1, 1)
plt.plot(frequency.squeeze(), S12_real, label='actual_real')
