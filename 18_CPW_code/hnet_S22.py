...
frequency_response_real = torch.Tensor(S22_real)
trained_frequency_response_real = torch.Tensor(S22_real[0:a])
testing_frequency_response_real = torch.Tensor(S22_real[a:totalSize])


frequency_response_imag = torch.Tensor(S22_imag)
trained_frequency_response_imag = torch.Tensor(S22_imag[0:a])
testing_frequency_response_imag = torch.Tensor(S22_imag[a:totalSize])
...
plt.plot(frequency.squeeze(), S22_real)
...
# %%
plt.figure()
plt.plot(frequency.squeeze(), S22_real, label='actual')
...
plt.subplot(2, 1, 1)
plt.plot(frequency.squeeze(), S22_real, label='actual_real')
...
plt.plot(testing_frequency[seq_len-1:-batch_size-1], y_imag_hat[a:]+0.15, label='predicted mean')
plt.fill_between(testing_frequency[seq_len-1:-batch_size-1], y_imag_hat_lower_bound[a:]+0.15, y_imag_hat_upper_bound[a:]+0.15, color='lightblue', label='95% Confidence region')
plt.grid(b=True)
plt.legend(loc='best')
plt.xlabel('frequency [GHz] ------>')
